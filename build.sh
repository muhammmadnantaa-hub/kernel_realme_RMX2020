...
# Clone SukiSU repo
if [ ! -d "KernelSU" ]; then curl -LSs "https://raw.githubusercontent.com/littlepe/SukiSU-Ultra/mainline/kernel/setup.sh" | bash -s susfs-main; fi
...
function KERNEL_COMPILE() {
	...
	if [[ ! -d "clang" ]]; then mkdir -p clang
		wget https://android.googlesource.com/platform/prebuilts/clang/host/linux-x86/+archive/167e11df8c330bced88cdf5808f61f41d9eab330/clang-r584948.tar.gz -O clang.tar.gz
